//
//  PVSTGPSLocationPicker.m
//  PVSDK
//
//  Copyright © 2017 PowerVision. All rights reserved.
//

#import "PVSTGPSLocationPicker.h"
...
@interface PVSTGPSLocationPicker () <CLLocationManagerDelegate, MBProgressHUDDelegate>
{
...
@implementation PVSTGPSLocationPicker

static PVSTGPSLocationPicker *_LocationPicker = nil;

+ (instancetype)shareGPSLocationPicker
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _LocationPicker = [[self alloc] init];
    });
...
+ (id)allocWithZone:(struct _NSZone *)zone
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _LocationPicker = [super allocWithZone:zone];
    });
    return _LocationPicker;
}
...
- (void)resetDefaultVariable
{
    ...
    _mode = GPSLocationPickerModeDeterminateHorizontalBar;
    _showWaitView    = YES;
    _showLocTime     = YES;
    _showDetailInfo  = YES;
}
...
{
    switch (self.mode) {
        case GPSLocationPickerModeDeterminateHorizontalBar:
            return MBProgressHUDModeDeterminateHorizontalBar;
            
        case GPSLocationPickerModeAnnularDeterminate:
            return MBProgressHUDModeAnnularDeterminate;
            
        case GPSLocationPickerModeIndeterminate:
            return MBProgressHUDModeIndeterminate;
            
        case GPSLocationPickerModeDeterminate:
...
- (void)startGetLocation
{
    ...
    [[PVSTGPSLocation shareGPSLocationPicker] startLocationAndCompletion:^(CLLocation *location, NSError *error) {
...
- (void)locationSuccess:(CLLocation *)coord
{
    ...
    [[PVSTGPSLocation shareGPSLocationPicker] stop];
    if (_locationResultBlock) {
